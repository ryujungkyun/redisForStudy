...
async function main() {
    ...
    try {
        ...
        // XX : 존재할 떄만 업데이트
        const xxResult = await client.zadd('leaderboard', 'XX', 150, 'Alice'); // zadd 이미 있는 경우에는 추가되지 않는다 0을 반환한다 
        ...
        const rankDesc = await client.zrank('leaderboard', 'Alice');
        console.log('Alice Rank (ZRANK):', rankDesc);

        // ZREVRANK : 높은 점수 기준 순위 (0부터 시작) 지레브랭크
        const renkDesc = await client.zrevrank('leaderboard', 'Alice');
        console.log('Alice Rank (ZREVRANK):', renkDesc);

        // ZINCBY : 점수 증가 여러개의 수치를 증가시킬 수 있다 
        const newScore = await client.zincrby('leaderboard', 30, 'Bob');
        console.log('Bob New Score (ZINCBY):', newScore);
